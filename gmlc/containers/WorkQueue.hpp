...
class workBlock : public basicWorkBlock
{
  public:
    ...
    {
        static_assert(std::is_same<decltype(newWork()), retType>::value,
...
class workBlock<void> : public basicWorkBlock
{
  public:
    ...
    workBlock(Func &&newWork) : task(std::forward<Func>(newWork)), loaded(true)
    {
        static_assert(std::is_same<decltype(newWork()), void>::value,
                      "work does not match type");
        reset();
    }
...
*/
    ...
    */
    void addWorkBlock(std::shared_ptr<basicWorkBlock> newWork,
    ...
    */
    ...
    {
        priorityRatio =
          (newPriorityRatio > 0) ? newPriorityRatio : defaultPriorityRatio;
    };
  ...
  private:
    ...
    std::condition_variable
      queueCondition;  //!< condition variable for waking the threads
