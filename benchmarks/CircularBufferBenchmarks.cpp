    ...
    for (int index = 0; index < read_size; ++index) {
        data_block[static_cast<std::size_t>(index)] =
            static_cast<unsigned char>(
                static_cast<unsigned int>(index) & 0xFFU);
    }
...
{
    unsigned char *block = new unsigned char[1520];  // 3x504+4  otherwise there
is a potential scenario in which
2
                                                     // 500byte messages cannot
fit CircularBufferRaw buf (block, 1520);
